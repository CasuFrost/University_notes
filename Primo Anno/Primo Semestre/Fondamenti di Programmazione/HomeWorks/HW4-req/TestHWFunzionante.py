...
def most_frequent_chars(filename: str) -> str:
    ...
    ListaStringhe = build_string_list(filename)
    
    return stringList_to_finalString(ListaStringhe)


def calcolo_media(filename,cicli):
    valori=[]
    for i in range(cicli):  
        start=time.time()
        most_frequent_chars(filename)
        valori.append(round(time.time()-start,2))
    return "tempo medio impiegato : "+str(round(sum(valori)/len(valori),2))+" - "+str(cicli)+" test eseguiti"

print(calcolo_media('test12/incipience.txt', 4))
