...
def elaborazione_ricorsiva_strutturaDati(istante,dizTurni,DizComandi,ValoriStruttura,triplaStruttura,listaIstantiFatti):    
    #global volte
    haveValidPos=False
    #if istante in listaIstantiFatti:
    for i in listaIstantiFatti:
        if i[0]==istante:
    ...
    for i in istante[ValoriStruttura["."]]:
        ...
        if nuovoIstante:
            haveValidPos=True
            elaborazione_ricorsiva_strutturaDati(nuovoIstante,dizTurni,DizComandi,ValoriStruttura,triplaStruttura,listaIstantiFatti)
            #listaIstantiFatti.append(nuovoIstante)
    ...
    if not haveValidPos:
        ...
        if len(istante[0])>len(istante[1]):
            triplaStruttura[0]+=1
            #listaIstantiFatti.append((istante,triplaStruttura))
...
def dumbothello(filename : str) -> tuple[int,int,int] :
    ...
    #print(listaIstantiFatti)
    elaborazione_ricorsiva_strutturaDati(istante,dizTurni,DizComandi,ValoriStruttura,triplaStruttura,listaIstantiFatti)
