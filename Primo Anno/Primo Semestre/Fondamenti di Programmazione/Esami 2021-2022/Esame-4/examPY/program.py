...
"""
def binaryConversion(listaBinary,value):
    if len(listaBinary)>1:
        a=listaBinary.pop(0)
        return binaryConversion(listaBinary,value+a*pow(2,len(listaBinary)))
    else:
        
        return value+listaBinary[0]
...
def rec3(S):
    ...
    for i in S :
        if int(i) == 0 or int(i)==1:
            listaBinary.append(int(i))
        if len(listaBinary)==3:
            print(binaryConversion(listaBinary,0))
            listaBinary=[]
             #genera numero
...
"""
import tree
def rec4(root,depth,lista,k):
    if root.value%k==0:
        lista.append(depth)
    if root.left!=None:
        rec4(root.left,depth+1,lista,k)
    if root.right!=None:
        rec4(root.right,depth+1,lista,k)
