...
\defi{} Sia $f(x) : I\subseteq\R\rightarrow \R$ una funzione, e sia $[a,b]\subseteq I$ un intervallo
...
\eqImportante{$\displaystyle\int_a^bf(x)dx$}
...
\begin{tikzpicture}
    \begin{axis}[
      ...
      xmin=-0.5, xmax=1.5,
   	ymin=-0.5, ymax=1.5,
      no marks,xtick distance=0.5, ytick distance=0.5,
      ]
      \addplot+[smooth,magenta,name path=A, domain=-3:3] {x}; % actual curve
      \addplot+[draw=none,name path=B] {0};     % “fictional” curve
      \addplot+[lightgray] fill between[of=A and B,soft clip={domain=0:1}]; % filling
    \end{axis}
  \end{tikzpicture}
\end{center}
...
$$
\teo{(linearità dell'integrale)} Siano $f$ e $g$ due funzioni integrabili su $[a,b]$, siano $\alpha$ e 
$\beta$ due costanti reali, vale che 
\eqImportante{
$
\displaystyle \int_a^b[\alpha\cdot f(x)+\beta \cdot g(x)]\;dx=
\alpha\cdot\int_a^bf(x)\;dx+\beta\cdot\int_a^bg(x)\;dx
$}
...
\teo{(integrale assoluto)}$$ | \int_a^bf(x)\;dx|\le \int_a^b|f(x)|\;dx$$\acc 
\teo{(monotonia dell'integrale)} Se $f\le g$ allora
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
          ...
          xmin=0, xmax=6,
           ymin=-3, ymax=3,
          no marks,xtick distance=1, ytick distance=1,
          ]
          \addplot[smooth,magenta,name path=A, domain=0:6] {1.5*sin(deg(2*x))}; % actual curve
          \addlegendentry{\(f(x)\)}
        \end{axis}
      \end{tikzpicture}
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot[smooth,red,name path=A, domain=0:6] {1.5*sin(deg(2*x))+1.5}; % actual curve
          \addlegendentry{\(f(x)+q\)}
        \end{axis}
      \end{tikzpicture}
    \end{center}
Si ha che 
