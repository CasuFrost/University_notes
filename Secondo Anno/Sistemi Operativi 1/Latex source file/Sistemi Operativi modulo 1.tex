...
\subsubsection{Semafori}\label{semafori}
...
Abbiamo quindi visto che i semafori hanno 3 scopi principali : \begin{itemize}
    ...
    \item Forzare le politiche di scheduling per eseguire i thread secondo uno specifico ordine. \ref{scheduleConstraint}
\end{itemize}
...
\subsubsection{Forzare lo Scheduling dei Thread}\label{scheduleConstraint}
In che modo un semaforo può forzare lo scheduler ad eseguire un insieme di thread secondo un determinato ordine? È 
possibile considerare un insieme di thread, in cui il thread \(X\), ossia quello che va eseguito per primo, eludendo 
le politiche di scheduling, implementa il metodo \code{signal()} alla fine delle sue istruzioni. Tutti gli altri processi 
vedranno il metodo \code{wait()} all'inizio della loro esecuzione. Impostando il valore del semaforo a 0, esclusivamente il 
processo \(X\) potrà venire eseguito, alla sua terminazione, richiamando il metodo \code{signal()}, cederà il controllo 
agli altri processi. 
Argomento correlato : Semafori \ref{semafori}
\end{document}