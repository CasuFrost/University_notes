...
(trattato nel corso di \color{blue}\href{https://github.com/CasuFrost/University_notes/blob/main/Secondo%20Anno/Primo%20Semestre/Basi%20di%20Dati%201/Latex%20source%20file/Basi%20di%20Dati%20modulo%201.pdf}{Basi di Dati 1}\color{black}).
\greybox{
...
\acc
Se $u_1,u_2\dots,u_k$ sono componenti fortemente connesse in $\nicefrac{G}{C}$, con
$C$ un insieme di vertici (ciclo) e $V_C$ il vertice contratto, si ha che le componenti in
...
ora l'algoritmo non lineare.\greybox{
    ...
    \hphantom{ident}\hphantom{ident}\code{return \{\{v\}|v $\in$ V(G)\}}\comm{ogni vertice è una componente}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{G=$\nicefrac{G}{C}$}\\
    \hphantom{ident}\code{$V_C$ = vertice contratto}\\
    \hphantom{ident}\code{($u_1,u_2\dots,u_k$)=Fort(G)}\comm{il grafo è stato modificato con una contrazione su C}\\
...
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{if(Dist[u]>Dist[v]+w(u,v))\{}\\
...
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{H.remove(v)}\\
...
\hphantom{ident}\code{if(A.length()==2)\{}\\
...
polinomiale, supponiamo di voler provare a fare ricerca bruta, se $n$ è il numero di file, dovremmo provare ogni singola combinazione
...
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{T[i,j]=max(T[i-1,j], T[i-1,j-$p_i$]+$v_i$)}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}else\{}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{T[i,j]=T[i-1,j]}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}}\\
\hphantom{ident}\hphantom{ident}\code{\}}\\
...
$$ \forall u \in N,\;\; P_u=\text{ percorso da }x\text{ a }u\text{ di $k$ archi}$$
...
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{for each u$\in$V(G) t.c. $\exists$(u,z)$\in$E(G)\{}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{if(T[u,k-1]+w(u,z)>massimo $\lor$ massimo==NULL)\{}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{massimo=T[u,k-1]+w(u,z)}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}}\\
\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{T[z,k]=massimo}\\
\hphantom{ident}\hphantom{ident}\code{\}}\\
\hphantom{ident}\code{\}}\\
...
procedere? Non basta aggiungere alla soluzione parziale semplicemente $A[i+1]$.\acc 
Si consideri un nuovo array $m'$, tale che $$m'[i]=\text{ soluzione del problema con input l'array }A[0:i]\text{ con il 
vincolo }A[i]\in m'[i]$$
Sarà tale array la chiave per trovare la soluzione, ad ogni passo de facto, ci sono due possibilità 
$$ m'[i+1]=\max(A[i+1], m'[i]+A[i+1])$$
Se la soluzione ottimale fosse $A[i:j]$, nell'algoritmo, essa sarà considerata al passo $j-esimo$, durante il calcolo di $m'[j]$.
\greybox{
    \code{intervallo\_max(A : array lungo $n$)\{}\\
    \hphantom{ident}\code{m = array lungo $n$}\\ 
    \hphantom{ident}\code{m[0]=A[0]}\\
    \hphantom{ident}\code{for (int i = 0; i<$n$;i++)\{}\\ 
    \hphantom{ident}\hphantom{ident}\code{m[i]=max(A[i],A[i]+m[i-1])}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{Sol = 0}\\
    \hphantom{ident}\code{for (int i = 0; i<$n$;i++)\{ Sol = max(Sol,m[i]) \}}\\
    \hphantom{ident}\code{return Sol}\\
    \code{\}}
}
Il seguente algoritmo, dato l'array $m'$ già riempito dall'algoritmo sovrastante, restituisce i valori degli indici 
\codee{start} e \codee{finish}.
\greybox{ 
    \code{intervallo\_max\_indici(m : array lungo $n$, Sol)\{}\\
    \hphantom{ident}\code{start = 0}\\
    \hphantom{ident}\code{finish = 0}\\
    \hphantom{ident}\code{int i}\\
    \hphantom{ident}\code{for (i = 0; i<$n$;i++)\{}\\
    \hphantom{ident}\hphantom{ident}\code{if( m[i]==Sol) \{}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{finish = i}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{break}\\
    \hphantom{ident}\hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{pref = A[i]}\\
    \hphantom{ident}\code{while ( pref $\ne$ Sol)\{}\\
    \hphantom{ident}\hphantom{ident}\code{i=i-1}\\
    \hphantom{ident}\hphantom{ident}\code{pref+=A[i]}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{start = i}\\
    \hphantom{ident}\code{return (start,finish)}\\
    \code{\}}
}
...
\begin{center}
    ...
    $y$ = L I 
\end{center}
Vi è in comune solo la lettere 'L'. Consideriamo adesso $M[i+1,j+1]=M[2,2]$, dovrebbe rappresentare la sottosequenza massima 
delle stringhe\begin{center}
    $x$ = A L B  \hphantom{spaziospazio}
    $y$ = L I B
\end{center}
Essendo che $x[i+1]=y[j+1]$, è possibile considerare il valore precedente della matrice, ossia  $M[1,1]=1$, ed incrementarlo di 
1, dato che è possibile considerare i due caratteri $x[i+1]$, $y[j+1]$ in comune nella soluzione $M[2,2]$.\acc 
\textbf{Risulta chiaro che} : Se $x[i]=y[j]$, allora $M[i,j]=M[i-1,j-1]+1$.\acc 
Nel caso in cui $x[i]\ne y[j]$, non sarà possibile incrementare la soluzione, in quanto non sono possibili nuovi 
accoppiamenti di caratteri, si avrà che: 
$$M[i,j]=\max(M[i-1,j],M[i,j-1],M[i-1,j-1]) $$
\greybox{ 
    \code{LCS(x,y)\{}\\
    \hphantom{ident}\code{n = x.lenght()}\\
    \hphantom{ident}\code{m = y.length()}\\
    \hphantom{ident}\code{M = matrice n $\times$ m}\\
    \hphantom{ident}\code{if(x[0]==y[0])\{ M[0,0]=1 \}}\\
    \hphantom{ident}\code{else \{ M[0,0]=0 \}}\\
    \hphantom{ident}\code{for(int i = 1;i < n;i++)\{}\\
    \hphantom{ident}\hphantom{ident}\code{if(x[i]==y[0])\{ M[i,1]=1 \}}\\
    \hphantom{ident}\hphantom{ident}\code{else\{ M[i,1]=M[i-1,1] \}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{for(int j = 1;j < m;j++)\{}\\
    \hphantom{ident}\hphantom{ident}\code{if(y[j]==x[0])\{ M[1,j]=1 \}}\\
    \hphantom{ident}\hphantom{ident}\code{else\{ M[1,j]=M[1,j-1] \}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{for(int i = 1;i < n;i++)\{}\\
    \hphantom{ident}\hphantom{ident}\code{for(int j = 1;j < m;j++)\{}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{if (x[i]==y[i])\{ M[i,j]=M[i-1,j-1]+1 \}}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{else \{ M[i,j] = max(M[i-1,j-1], M[i,j-1], M[i-1,j]) \}}\\
    \hphantom{ident}\hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{return M[n,m]}\\
    \code{\}}
}
Si veda il seguente esempio di applicazione dell'algoritmo e costruzione della matrice:\begin{center}
    $M = $\begin{tabular}{l|llllll}
        & 1 & 2 & 3 & 4 & 5 & 6 \\ \hline
      1 & 0 & 1 & 1 & 1 & 1 & 1 \\
      2 & 0 & 1 & 1 & 1 & 1 & 1 \\
      3 & 0 & 1 & 2 & 2 & 2 & 2 \\
      4 & 0 & 1 & 2 & 2 & 3 & 3 \\
      5 & 0 & 1 & 2 & 2 & 3 & 3
      \end{tabular} $\;\;\;\;\;\;\;\;\;\;\;\;\begin{matrix}
        x = \text{ A L B E R O}\\ 
        y = \text{ L I B R I}
      \end{matrix}$
\end{center}
Data la matrice $M$ sulla quale è stato applicato l'algoritmo, come si può ottenere la sottosequenza comune massima? Sarà 
necessario iniziare dall'elemento $M[i,j]$ con $i=n,\;j=m$, per poi spostarsi verso sinistra (diminuendo il valore 
dell'indice $i$), ogni qual volta vi sarà una variazione del valore della matrice all'elemento $i$, considereremo il carattere 
della prima stringa contenuto nel medesimo indice.
\greybox{
    \code{LCS\_str(M : matrice n$\times$m, x, y)\{}\\ 
    \hphantom{ident}\code{Sol = []}\comm{stringa vuota}\\
    \hphantom{ident}\code{for(int i = n-1; i>0;i--)\{}\\
    \hphantom{ident} \hphantom{ident}\code{if(M[i,m]$\ne$M[i-1,m])\{}\\
    \hphantom{ident} \hphantom{ident} \hphantom{ident}\code{Sol.insert(x[i])}\comm{inserimento alla prima posizione}\\
    \hphantom{ident} \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{if(M[0,m]==1)\{ Sol.insert(x[1]) \}}\\
    \hphantom{ident}\code{return Sol}\\
    \code{\}}
}
...
\end{itemize}
In \textbf{conclusione} : $$ 
M[i,j]=\begin{cases}
    \min(M[i-1,j]+1,\;M[i,j-1]+1,\;M[i-1,j-1]+1) \text{ se }x[i]\ne y[j]\\ 
    \min(M[i-1,j]+1,\;M[i,j-1]+1,\;M[i-1,j-1])\text{ altrimenti }
\end{cases}
$$\greybox{
    \code{edit\_distance(x,y)\{}\\
    \hphantom{ident}\code{n=x.length()}\\
    \hphantom{ident}\code{m=y.length()}\\
    \hphantom{ident}\code{M = matrice (n+1)$\times$(m+1)}\\
    \hphantom{ident}\code{M[0,0]=0}\\
    \hphantom{ident}\code{for(int i = 1;i$\le$n;i++)\{ M[i,0] = i \} }\\
    \hphantom{ident}\code{for(int j = 1;i$\le$m;j++)\{ M[0,j] = j \} }\\
    \hphantom{ident}\code{for(int i = 1;i$\le$n;i++)\{}\\
    \hphantom{ident}\hphantom{ident}\code{for(int j = 1;i$\le$m;j++)\{}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{if(x[i]==y[j])\{}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{M[i,j] = min(M[i-1,j]+1, M[i,j-1]+1, M[i-1,j-1])}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}else \{}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\hphantom{ident}\code{M[i,j] = min(M[i-1,j]+1, M[i,j-1]+1, M[i-1,j-1]+1)}\\
    \hphantom{ident}\hphantom{ident}\hphantom{ident}\code{\}}\\
    \hphantom{ident}\hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{\}}\\
    \hphantom{ident}\code{return M[n,m]}\\
