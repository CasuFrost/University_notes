...
ed è una funzionalità di base dell'Internet, sebbene essa è implementata a livello di applicazione, tramite il
...
comulativo, ne viene inviato uno \textbf{selettivo}, riservato ad ogni singolo pacchetto, i pacchetti arrivati ma 
...
e riduce il grado di ridondanza, inoltre se la reta è congestionata, la rispedizione di tutti i pacchetti 
...
permettendo a due enti di comunicare e contemporaneamente notificarsi dell'ascolto.
...
flusso di byte che compongono i messaggi, che deve arrivare dal mittente al destinatario in un 
...
che per un messaggio di ACK (dato che utilizza il piggy-backing), ed è la seguente:\begin{center}
...
Il campo \textit{"finestra del ricevitore"} indica il numero di byte che il destinatario può "accogliere", ossia 
attendere, è necessario per il controllo della congestione, se la rete è congestionata, tale numero 
...
il tempo di RTT effettivo misurato dal timer $sampleRTT$, tramite tale misurazione per un pacchetto,
 ...
 Dove $\beta$ è un numero reale fra 0 ed 1. Il tempo effettivo del timeout sarà:
 $$ timeout=RTTstimato+4\cdot DevRTT$$
 Si noti il seguente grafico che mostra la differenza fra il RTT misurato ed il RTT stimato (quest'ultimo risulta 
...
invia un ACK comulativo con numero 6 (si ricordi che il numero di segmento si riferisce ai byte e non al segmento 
...
di ricezione.\acc 
Si osservi come questa è una congestion \textit{locale}, e non da parte della rete, può avvenire anche se la rete 
