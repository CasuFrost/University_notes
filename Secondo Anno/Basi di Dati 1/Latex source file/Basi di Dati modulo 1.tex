...
\subsubsection{L'Algoritmo  (\codee{Compute }\(X^+_F\)\codee{ from }\(F\))}\label{alg}
...
\subsubsection{L'Algoritmo (\codee{Compute }\(X^+_G\)\codee{ from }\(F\))}
...
definito \(Z_{i+1}\), ciò significa che \(A\in Z_i\) oppure \(A\in S_i\). Il primo caso dimostra di per se la tesi, si consideri quindi 
il caso in cui \(A\in S_i\). Ricordo che \(S_i=\bigcup_{j=1}^k(Z_i\cap R_j)_F^+\cap R_j\), allora, \(\exists j\in\{1,2\dots,k\}|
...
Sappiamo che \(X\subseteq Z_f\), posso fare un osservazione : Se \(X\subseteq Z_f\), allora sicuramente 
\(X^+_G\subseteq (Z_f)_G^+\), fatta tale osservazione, mi basta dimostrare che \(Z_f=(Z_f)_G^+\). Anche se calcolare \(G\) 
richiede tempo esponenziale, questa è una dimostrazione, quindi si può assumere di conoscere \(G\). Applichiamo il normale 
algoritmo \codee{Compute }\(X^+_F\)\codee{ from }\(F\) visto nel paragrafo \ref{alg} per calcolare la chiusura di \(Z_f\), applicando 
tale algoritmo, si ricostruisce iterativamente \(S\), si ha la prima riga di codice definita in tal modo \(S'=\{A|\exists Y\rightarrow V\in G,Y\subseteq Z_f\land A\in V\}\)
Si ricordi che \(G=\displaystyle\bigcup^n_{i=1}\pi_{R_i}(F)\), 
ciò vuol dire che \(\exists j \in \{1,2\dots,k\}|Y\rightarrow V\in \pi_{R_j}(F)\), 
Se \(A\in V\), allora \(A\in R_j\), e \(Y\rightarrow V\in F^+\), dato che \(G\subseteq F^+\). Noto che :\(\begin{cases}
    Y\rightarrow V\in F^+\\
    A\in V
\end{cases}\implies A\in Y_F^+\).
Ricordando la definizione di \(\pi_{R_j}(F)\), so che, se \(A\in V\) e \(Y\rightarrow V\in \pi_{R_j}(F)\), allora 
sicuramente \(A\in R_j\). A questo punto : \(\begin{cases}
    A\in R_j\\ 
    A\in Y_F^+\text{ e }Y\in Z_f \implies A\in (Z_f)^+_F
\end{cases}\implies A\in(Z_f\cap R_j)^+_F\cap R_j\subseteq S_f\implies A\in S_f\) ma \(S_f\in Z_f\) quindi 
\(A\in Z_f\). Questo, vuol dire che, all'inizio, in \(S\) vengono inseriti solo elementi che sono già 
in \(Z_f\), quindi (si guardi l'algoritmo), essendo che \(S\subseteq Z_f\), non viene eseguito il \codee{while}, e l'algoritmo 
termina, dando in output lo stesso elemento che è stato preso in input, ciò, significa che la chiusura dell'elemento 
è uguale all'elemento stesso, e quindi \(Z_f=(Z_f)^+_G\). Avendo iniziato il discorso dicendo che \(X^+_G\subseteq (Z_f)_G^+\), 
ne concludiamo che  \(X_G^+\subseteq Z_f\). \(\blacksquare\)
\end{document}
