...
\(dfrac{n}{2}\) blocchi prima di trovare quello ricercato. I blocchi, vengono sempre cercati ed identificati tramite il campo 
...
Dato uno schedule \(S\) di \(n\) transazioni, costruiamo un grafo \(G\) nel seguente modo : \begin{itemize}
    \item I nodi di \(G\) sono le transazioni \(T_1,T_2\dots,T_n\) dello schedule \(S\).
...
In questo nuovo modello, il grafo \(G\) viene costruito in tal modo : \begin{itemize}
    \item I nodi di \(G\) sono le transazioni \(T_1,T_2\dots,T_n\) dello schedule \(S\).
    \item Ci sarà un arco \(T_i\rightarrow_X T_j\) con etichetta \(X\) in \(S\) se si verifica una delle due seguenti : \begin{itemize}
