...
\begin{dimostrazione}
    ...
    [b]\subseteq[a]\). Essendo \( [b]\subseteq[a]\) e \( [a]\subseteq[b]\), necessariamente \([a]=[b]\).
\end{dimostrazione}
...
\begin{enumerate}
    ...
    \item \(\forall k\text{ tale che } n=k\cdot c\) per qualche \(c\) (per ogni \(k\) divisore di \(n\)),
...
di \(\left\langle g^{\frac{n}{k}}\right\rangle\). (Dimostrazione \ref{dimStrutCicl}) Quindi, il teorema di struttura dei gruppi ciclici si 
...
\begin{figure}[h]
    \centering{
    \includegraphics[width=0.5\textwidth ]{images/vector3.eps}
    }
\end{figure}
...
lineare, \(T(\bar 0_V)=\bar 0_W\), si ha che \(T(\ve)=T(\bar 0_V)\), essendo però \(T\) iniettiva, \(\ve=\bar 0_V\). 
...
Procediamo con la dimostrazione. \begin{enumerate}
    ...
    \(\varphi^{-1}(\Z)=G\), oppure   \(\varphi^{-1}(\Z_n)=G\), se \(H\) è un sottogruppo di \(G\) ed un isomorfismo 
    mantiene le operazioni, \(\varphi^{-1}(\Z)\) o \(\varphi^{-1}(\Z_n)\) associa a \(\Z\) o \(\Z_n\) un sottogruppo di \(G\), in questo caso \(H\). Quindi un sottogruppo 
...
\raggedleft\(\blacksquare\)\\
\raggedright
\subsubsection{Dimostrazione del Teorema Cinese del Resto}
Riguardo il seguente sistema :
$$\begin{cases}
    x=c_1 \mod r_1\\
    x=c_2 \mod r_2\\
    \dots\\ x=c_s \mod r_s
\end{cases}$$
Considero un numero \(R=r_1\cdot r_2\dots r_s\), considero poi per ogni \(k-esima\) equazione, 
il numero \(R_k=\nicefrac{R}{r_k}\), essendo che i valori \(r_1,r_2\dots,r_s\) sono co-primi fra loro, 
si avrà che \(R_k\) e \(r_k\) sono co-primi, ossia \(MCD(R_k,r_k)=1\). \\
Ricordando che le soluzioni di un equazione congruenziale \(ax=b \mod n\) sono in numero \(MCD(a,n)\), si avrà 
che l'equazione $$R_kx=c_k \mod r_k$$
ammette un unica soluzione \(\mod r_k\). Denotiamo tale soluzione \(\bar x_k\), quindi si ha 
la seguente identità, per ogni equazione, \(\bar x_k \cdot R_k = c_k \mod r_k\). Tornando al sistema quindi, abbiamo 
trovato una soluzione diversa per ogni equazione, ossia \(\bar x_k \cdot R_k\). Si avrò quindi che 
$$\begin{cases}
    x=c_1 \mod r_1\spaz \text{ ha soluzione }\bar x_1R_1\\
    x=c_2 \mod r_2\spaz \text{ ha soluzione }\bar x_2R_2\\
    \dots\\ x=c_s \mod r_s\spaz \text{ ha soluzione }\bar x_sR_s
\end{cases}$$
Facciamo però un \textbf{osservazione cruciale}, sappiamo che \(R_k=\nicefrac{R}{r_k}\), ossia : \\
 \(\dfrac{r_1\cdot r_2\dots r_{k-1},r_{k+1}\dots r_s}{r_k}\), ma ciò significa che, \(R_k\), è multiplo di tutti gli argomenti 
 dei moduli del sistema tranne che di \(r_k\). Ciò significa che \(R_i=0 \mod r_k\spaz \forall i\ne k\). Consideriamo quindi 
 la seguente soluzione per il sistema \(\bar x=\bar x_1R_1+\bar x_2R_2\dots\bar x_sR_s\): 
 $$\begin{cases}
    x_1R_1+\bar x_2R_2\dots\bar x_sR_s=c_1 \mod r_1\\
    x_1R_1+\bar x_2R_2\dots\bar x_sR_s=c_2 \mod r_2\\
    \dots\\
    x_1R_1+\bar x_2R_2\dots\bar x_sR_s=c_k \mod r_k
    \\\dots\\x_1R_1+\bar x_2R_2\dots\bar x_sR_s=c_s \mod r_s
\end{cases}$$
Ma per l'osservazione cruciale riguardo l'annullamento dei termini, ciò diventa : 
$$\begin{cases}
    x_1R_1=c_1 \mod r_1\\
    \bar x_2R_2=c_2 \mod r_2\\
    \dots\\
    x_kR_k=c_k \mod r_k
    \\\dots\\\bar x_sR_s=c_s \mod r_s
\end{cases}$$E sappiamo che queste sono soluzioni del sistema, ne deduciamo che \(\bar x=\bar x_1R_1+\bar x_2R_2\dots\bar x_sR_s\)
risolve il sistema, è quindi una soluzione (unica). \(\blacksquare\)
\subsection{Sull'Algebra Lineare}
\subsubsection{Teorema Fondamentale per i Sistemi Lineari}
Sia \(A\bar x = \bar b\) un sistema e \(S\bar x = \bar c\) una sua riduzione a scala  :\begin{enumerate}
    \item Le soluzioni di \(A\bar x = \bar b\) sono le stesse di \(S\bar x = \bar c\).
    \item \(KerA=KerS\)
    \item Le immagini in generale non sono identiche ma \(\rg(A)=\rg(S)\)
    \item Le colonne di \(S\) con i pivot sono le colonne corrispondenti in \(A\) linearmente indipendenti, che 
    generano quindi \(Im(A)\).
\end{enumerate}
\textbf{Dimostrazione} : La \((1)\) e la \((2)\) seguono dal fatto che i due sistemi sono equivalenti in quanto si 
applica il metodo di Gauss. La \((3)\) è vera perché $$\rg(A)=\dim(V)-\dim(KerA)=\dim(V)-\dim(KerS)=\rg(S)$$.
(4) Bisogna dimostrare che le colonne in \(A\), ossia \(A^{j_1},A^{j_2}\dots,A^{j_r}\), date dai pivot di \(S\) sono
 linearmente indipendenti, quindi che il sistema \(x_1A^{j_1}+x_2A^{j_2}\dots+x_rA^{j_r}=\bar 0\) ha come unica soluzione 
 \(\bar 0\). Riducendo a scala questo nuovo sistema per dimostrarne l'indipendenza, otterremo un sistema 
 del tipo  \(x_1S^{j_1}+x_2S^{j_2}\dots+x_rS^{j_r}=\bar 0\) con le colonne di \(S\) contenenti i pivot, ma sappiamo che 
 le colonne con i pivot di \(S\) sono linearmente indipendenti, quindi l'unica soluzione è \(\bar 0\) e le colonne originali 
 in \(A\) sono linearmente indipendenti. \(\blacksquare\)
 \newpage
 \textbf{Nota} su un esercizio di esame : \\
 \textit{Osservazione} : \((g^{-1}hg)^{-1}=g^{-1}h^{-1}g\) Verifica : \(g^{-1}hg*g^{-1}h^{-1}g=g^{-1}hh^{-1}g=g^{-1}g=1\)\\
 Siano \(g^{-1}hg\in H^g\) e  \(g^{-1}h'g\in H^g\) : 
 $$g^{-1}hg*(g^{-1}h'g)^{-1}=g^{-1}hg*g^{-1}h'^{-1}g=g^{-1}hh'^{-1}g$$
 ma \(h*h'^{-1}\in H\) perché \(H\) è un sottogruppo quindi : \(g^{-1}hh'^{-1}g\in H^g\)
