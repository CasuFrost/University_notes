...
Esiste \(h\ge 1\in\N\) tale che \(P(\lambda)=(\lambda-\lambda_0)\cdot f(\lambda)\), dove \(f\) è una funzione 
tale che \(f(\lambda_0)\ne0\). Tale numero naturale \(h\) è detto \textit{molteplicità algebrica} di \(\lambda_0\), ed 
è denotato \(m(\lambda_0)\). Differentemente, 
...
campo \(\K\), e sia \(T:V\rightarrow V\) un applicazione lineare, con \(\dim(V)=n\), le seguenti sono equivalenti :\begin{itemize}
    \item \(T\) è diagonalizzabile.
    \item Esiste una base di \(V\) costituita da autovettori per \(T\).
    \item  per ogni 
    autovalore, si ha che la sua molteplicità algebrica è identica alla molteplicità geometrica.
    \item Siano \(\lambda_1,\lambda_2\dots,\lambda_k\) gli autovalori \textit{distinti}, si ha che :
    $$\sum_{i=0}^k\lambda_i=\dim(V)=n$$ Ossia che la somma delle molteplicità algebriche dei suoi autovalori è 
    \(n\).
