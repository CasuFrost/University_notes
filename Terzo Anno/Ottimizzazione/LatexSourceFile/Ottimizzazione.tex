...
\noindent\textbf{Caso base}: Se $m=0$, allora per ogni arco $(u,v)$ in $\vec G$, si ha $f(u,v)=0$, non essendoci archi in $W$ il numero di cammini edge-disjoint è 0, proprio come il valore massimo del flusso.
...
Per ogni insieme $W$ contenente un numero dispari di vertici si ha che $$ \begin{matrix}
    \displaystyle \sum_{e\in\delta(W)}\bar x^1_e &=&  \sum_{e\in\delta(W)}\bar x_e & \ge 1\\  
    \text{ in }G_1 & & \text{ in }G
\end{matrix}$$ 
