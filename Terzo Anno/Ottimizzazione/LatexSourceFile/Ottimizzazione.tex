...
\begin{definizione}
    si definisce \textbf{capacità di taglio} per un'insieme di taglio $\mathcal U$, la somma delle capacità degli archi che collegano i vertici in $\mathcal U$ ai vertici in $V(G)\backslash \mathcal U$
...
\end{figure}
\begin{osservazione}
    il valore massimale del flusso per una network è limitato dalla capacità di taglio di un qualsiasi insieme di taglio
    $$ \text{val}(f)\le c_t$$
\end{osservazione}
...
Nonostante ciò, esiste sempre un insieme $\mathcal U$ contenente $s$ e non $t$ la cui capacità di taglio è uguale al valore del flusso ottimale per la network data, tale insieme può essere trovato adoperando l'algoritmo di Ford-Fulkerson nella procedura precedentemente elencata. L'insieme di taglio con la capacità minore possibile, ha somma delle capacità uguale al valore del flusso massimale per una network, si può dire che  trovare il \textit{taglio minimo} equivale a trovare il \textit{flusso massimo}, questi due problemi costituiscono una coppia di programmi \textbf{primario-duale}, la definizione e formalizzazione verrà fornita in seguito nel capitolo \ref{dualità}.
\begin{osservazione}
...
\begin{figure}[h!]
    \centering 
    \includegraphics[width=0.65\textwidth ]{images/edgeDisjoint.eps}
    \caption{3 cammini edge-disjoint da $s$ a $t$}
\end{figure}
...
\textit{Esempio} : Si consideri il triangolo delineato dai punti $(0,0,3),(0,3,0),(3,0,0)$, tutti i punti di tale triangolo sono i vettori $\mathbf x\ge \mathbf 0$ che soddisfano $A\mathbf x = \mathbf b$.\begin{figure}[h]
    ...
    % Draw the triangle faces (you might want to adjust opacity for better visualization)
    \fill[blue!20, opacity=0.7] (A) -- (B) -- (C) -- cycle;
    ...
    \foreach \point/\label in {A/{(3,0,0)}, B/{(0,3,0)}, C/{(0,0,3)}}
        \node[circle, fill, inner sep=1.5pt] at (\point) {};
    \node[anchor=north east] at (A) {$(3,0,0)$};
    \node[anchor=north west] at (B) {$(0,3,0)$};
    \node[anchor=south] at (C) {$(0,0,3)$};
...
\begin{definizione}
    ...
    Una soluzione ammissibile $\bar{\mathbf x}$ è detta \textbf{basica} (BFS) se esiste un sotto-insieme di indici $\mathcal B\subseteq \{1,2,\dots n\}$ tale per cui\begin{itemize}
        ...
        \item $A_{\mathcal B}\bar\x_{\mathcal B}=\bb$
    \end{itemize}
\end{definizione}
...
\begin{osservazione}
    Ogni base ammissibile ha almeno una BFS associata.
\end{osservazione}
...
\end{definizione}
\begin{definizione}
    Dato un poliedro $P$, una \textbf{faccia} è un'insieme $\mathcal U\subseteq P$ tale per cui esiste un iperpiano $\mathcal X$ tale che $\mathcal X\cap P = \mathcal U$, e per cui, ogni punto di $P$ si trova in uno dei due mezzi spazi definiti da $\mathcal X$. Un vertice è una faccia di cardinalità 1.
\end{definizione}
...
$$
e 
$$ a_1x_1+\dots +a_nx_n<b, \ \ \ \forall \mathbf x\in P\backslash\{\mathbf v\} $$
