...
public:
    ...
    void time_step_action()
    {
        ...
        {
            current_state = randi_range(0, current_state);
        }
    ...
    double simulate_process(int cost_type)
    {
        ...
        double tmp_cost = 0;
        int time_passed = 0;
        while (current_state != end_state && current_state != end_state)
        {
            time_passed++;
            tmp_cost += time_step(cost_type);
        }
        // cout << "days " << time_passed << "\n";
        current_state = start_state;
        if (cost_type == 3)
        {
            return time_passed;
    ...
    void set_start_state(int i = 0) { start_state = i; }
    void set_end_state(int i) { end_state = i; }
    int get_start_state() { return start_state; }
    int get_end_state() { return end_state; }
};
