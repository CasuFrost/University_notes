...
Con $0^n1$ si intende una stringa che abbia un numero naturale di $0$ (quindi, almeno uno), e 
che termini con $1$ ( \color{red} la consegna è un altra in realtà, correggere \color{black}).
\begin{figure}[h!]
    \centering
    \begin{tikzpicture} [node distance = 3cm, on grid, auto]
        \node (q0)[state, left, initial] {$q_0$};
