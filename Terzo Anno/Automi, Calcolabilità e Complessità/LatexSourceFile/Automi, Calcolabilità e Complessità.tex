...
$w\in L(D)$, ed una sua decomposizione in 3 stringhe concatenate $w=xyz$. Esiste un intero $p\le|w|$, denotato \textg{pumping } 
...
Tale sequenza è lunga $n+1\ge p+1$ stati, fra i primi $p+1$ elementi c'è necessariamente uno stato ripetuto, sia 
 ...
 \end{itemize}\begin{enumerate}
    ...
    \item Essendo che $j<l$, allora la dimensione minima di $y$ è 1, in quanto il valore minimo che può assumere $l$ è $j+1$, ne consegue che $|y|=l-1-(j-1)=l-j>0$. 
...
Possono esserci differenti alberi di computazione, il comportamento del PDA può essere descritto dal seguente 
algoritmo 
\greybox{
    $\diamond $ si inserisce $\$ S$ nella pila \\ \\\color{blue}
...
\begin{center}
    \begin{tikzpicture} [node distance = 3cm, on grid, auto]
        ...
        \node (q2)[state,accepting, right = of q1] {$q_{accept}$};
        \path [-stealth, thick]
...
\end{center}
Gli stati $E=\{q_1,q_2,q_3,q_4\}$ sono quelli di transizione che nella notazione abbreviata verrebbero 
...
se\begin{enumerate}
    \item Inizia con la pila vuota, e termina la computazione sempre con la pila vuota 
...
\dimo{ claim 2} Verrà dimostrato per induzione sul numero di passi di computazione dell'automa $P$.\begin{itemize}
    ...
    computazione, precisamente, ci sono due possibili casi \begin{itemize}
        ...
        $k-1$ passi. Ricapitolando\begin{itemize}
            ...
            \item $y$ porta da $r$ ad $s$ lasciando la pila inalterata
...
\dimo{ claim } Si dimostra per induzione su $i$\begin{itemize}
    ...
    \item \textbf{passo induttivo} : si consideri un cammino lungo $k=i+1 $ essendo $i>1$ allora $k\ge 3$, la prima regola applicata 
...
\end{center}
In generale (senza entrare troppo nel formalismo) è chiaro come $\forall i \ uv^ixy^iz\in L$ (punto $(i)$ dimostrato). La dimostrazione del 
...
$$\begin{matrix}
 ...
 dove \begin{itemize}
    \item $C_1=q_0w$ 
    \item $\forall i \ \ \ C_i$ produce $C_{i+1}$
 ...
 decisore per $L$, ossia, accetta ogni stringa in $L$, e rifiuta ogni stringa che non è in $L$. Si dice che $M$ \textit{decide} $L$.\acc 
...
$M$ accetta, altrimenti si incremente l'indice $i$
 ...
 \begin{itemize}
    ...
    \item Se nessun ramo accetta o rifiuta ($N$ va in loop), allora anche $M$ andrà in loop
 \end{itemize}
...
La diagonalizzazione è una tecnica utilizzata per dimostrare alcune semplici proprietà degli insiemi. Sappiamo che un insieme generico $A$ è numerabile se esiste una biezione da $\N$ a $A$. Ad esempio, l'insieme dei numeri pari $E$ è numerabile perché esiste $$ f:\N \rightarrow E\ \ \ | \ \ \  f(n)=2n$$ che è una biezione.\acc 
...
\dimo{2} Sia $\mathcal{L}=\mathcal{P}(\Sigma^*)$ l'insieme di tutti i linguaggi le cui stringhe sono binarie. Un generico linguaggio $L$ è sotto-insieme di $\Sigma^*$ dove $\Sigma=\{0,1\}$. 
...
\end{enumerate}
Quindi $S$ è un decisore per tutte le coppie $<M,w>\in A_{TM}$. Ma $A_{TM}$ non è decidibile, è quindi impossibile che esista un decisore per $HALT_{TM}$.\hfill$\blacksquare$\acc 
Si consideri 
...
\defi{} Una funzione $f:\Sigma^* \rightarrow \Sigma^*$ si dice \textbf{calcolabile} se esiste una TM che su input $w\in \Sigma^*$ termina scrivendo sul nastro $f(w)$. Spesso tale TM ha un nastro dedicato per l'output della funzione.\acc 
...
\begin{enumerate}
    \item  Su input $<M,w>$, definisce due TM ausiliarie $M_1,M_2$, tali che \begin{itemize}
        ...
        \item $M_2$ accetta ogni input $x$ solo se $M(w)$ accetta, altrimenti rifiuta sempre.
    \end{itemize}
...
si definisce poi una TM $R$, il cui comportamento è descritto come segue\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
In pratica le due TM considerano ogni possibile stringa cercando una dimostrazione 
per l'affermazione in input. Si definisce il linguaggio 
$$ L_{provable}=\{<x>\ | \ x \text{ è dimostrabile }\}$$
Alcune osservazioni
\begin{itemize}
    \item Se $\Pi$ è valido, l'output di $P$ è vero, anche se non è assicurato che termini
...
\dimo{} Si assume che $\Pi$ sia consistente e completo, si definisce un'apposita TM $D$ il cui comportamento è descritto come segue \begin{enumerate}
...
è dimostrabile. Si considerano i due casi separatamente\begin{itemize}
    \item Se "$D(<D>)=\infty$" è dimostrabile, allora, per costruzione di $D$, essa stessa con input se stessa, ossia $D(<D>)$, trova una dimostrazione e termina, quindi $$D(<D>)\ne\infty$$ allora per il \textit{Lemma} esiste una dimostrazione per $D(<D>)\ne\infty$, quindi $D(<D>)\ne\infty$ è dimostrabile. Si ha che sia 
    $$ \begin{cases}
    D(<D>)=\infty\\ 
    D(<D>)\ne \infty
\end{cases}$$
...
Ma come si è mostrato in precedenza, $D(<D>)=\infty$ è un affermazione non dimostrabile, quindi è necessariamente vero che anche l'affermazione che la implica, ossia "$\Pi$ consistente" sia indimostrabile.\hfill$\blacksquare$
...
\textbf{Esempio} : Si consideri la TM multi nastro, si vuole calcolare l'overhead quando si considera una sua simulazione con una TM classica. \begin{center}
    \includegraphics[width=\textwidth ]{images/multinastro.eps}
\end{center}
...
Il teorema di \textit{gerarchia dei tempi}, che verrà formalizzato e dimostrato in seguito, afferma che la classe $P$ è un sottoinsieme stretto della classe $EXP$.\begin{center}
...
\prop{} ogni formula logica può essere scritta in forma normale congiuntiva. Si può quindi assumere (senza perdita di generalità) che una formula sia in CNF.\acc 
...
\acc \textbf{Dimostrazione del Fatto} : Supponiamo che in $G$ ci sia $(a,b)$ e che $a=1$ e $b=0$, allora in $\phi$ esiste la clausola $$\bar a \lor b \equiv a\rightarrow b$$
...
Ma attenzione \begin{itemize}
    ...
    \item vuol dire che $\bar b$ è in una componente antecedente a $\bar a$, oppure nella sua stessa componente
...
\end{itemize}
Quindi $\bar b$ è in una componente sia seguente che antecedente (o identica) a quella di $\bar a$, è una contraddizione, quindi il fatto è vero.\hfill$\square$\acc 
Sapendo che il fatto è vero, se per ogni variabile $x$, questa e la sua negata \underline{non sono} nella stessa componente fortemente connessa, allora l'assegnamento visto (in funzione dell'ordinamento topologico) soddisfa $\phi$ perché un arco $(a,b)$ equivale a $\bar a \lor b$, ma essendo impossibile che $a=1$ e $b=0$, ogni clausola $\bar a \lor b$ non sarà mai uguale a zero.\hfill$\blacksquare$\acc 
...
\defi{} Dato un linguaggio $L$, una TM $V$ è detta \textbf{verificatore} per $L$ se, prende in input due elementi (uno di questi in $L$) e \begin{itemize}
...
\boxedMath{$\implies$} Se $L$ ha un verificatore polinomiale $V$, è possibile definire una NTM $N$ la cui procedura è descritta come segue:\begin{itemize}
    \item su input $x$
    \item $N$ utilizzando il non determinismo, considera ogni possibile soluzione $y$ tale che $|y|=poly(|x|)$, e ciò, su ogni ramo di computazione, ha costo polinomiale
...
$$ x\in A \iff R_2(R_1(x))\in C$$\hfill$\blacksquare$\acc  
...
\begin{itemize}
    \item \textg{l'assegnamento delle variabili rappresenta una qualsiasi possibile tabella di computazione }
    \item \textg{se la formula è soddisfacibile, allora esiste una tabella accettante, quindi $N$ su input $x$ accetta  }
    \item \textg{in questo modo si esegue la riduzione da $A$ a $SAT$ }
\end{itemize}
...
questa formula serve a descrivere il fatto che la prima riga di una tabella valida deve necessariamente contenere la configurazione iniziale, che sappiamo essere 
...
questa formula assicura che la riga $l+1$ è coerente con la riga $l$, ossia che le due configurazioni che si susseguono sono coerenti e in accordo con la funzione di transizione $\delta$ della NTM $N$. Per fare ciò, basta controllare ogni possibile finestra su due righe, questa deve essere \textit{lecita} (non violare le regole di $\delta$).\begin{itemize}
    ...
    \item i simboli adiacenti ad uno stato $q$ devono evolvere nella riga successiva seguendo uno dei possibili risultati della $\delta$
...
\end{matrix}$$
$$ \phi_{move}=\bigwedge\limits_{1 \leq i,j \leq n^k}
...
\end{enumerate}
Essendo che ogni problema $NP$ si riduce a $SAT$, si ha che se $P=NP$, una soluzione valida per ogni problema $NP$ può essere trovata in tempo polinomiale, questo fatto avrebbe gravi conseguenze, infatti la fattorizzazione dei numeri interi è un problema $NP$, e la sua difficoltà nel risolverlo è alla base della crittografia.\acc 
...
Visto che per ipotesi $P=NP$, si ha che $L'\in P$, allora esiste una TM deterministica $M'$ che decide $L'$ in tempo polinomiale. Allora è possibile decidere $L$ in tempo esponenziale\begin{itemize}
    ...
    \item avvio $M'(x')$, questa opera in tempo polinomiale, se $M'$ accetta, allora $x\in L$, altrimenti rifiuta 
\end{itemize}
...
Ad esempio, esiste il linguaggio $$ UNSAT=\overline{SAT}$$
...
\begin{center}
    \includegraphics[width=0.4\textwidth ]{images/relCongetturata.eps}
\end{center}
...
siccome $\overline A$ è in $NP$ e $SAT$ è $NP$ completo, è vero che $\overline A$ si riduce a $SAT$, quindi $A$ si riduce ad $UNSAT$, quest'ultimo è quindi $coNP$ completo.\hfill$\blacksquare$\acc  
...
La TM userà un numero costante di nastri, ad alto livello la procedura è descritta dai seguenti punti\begin{itemize}
    ...
    \item si memorizza l'indice sul nastro 2 e si incrementa ad ogni passo, tale operazione richiede sempre complessità spaziale logaritmica 
...
\textbf{Esempio} : $3SAT\in PSPACE$, questo è certo perché è possibile provare ogni singolo assegnamento della formula, essendo $n$ il numero dei letterali, ad ogni assegnamento provato si usano al più $n$ celle. 
Essendo che $3SAT$ è $NP$ completo, ne consegue  che 
\sapbox{\centering$NP\subseteq PSPACE $}
\flowerLine
...
\dimo{} Sia $L\in DTIME(f(n))$, e sia $M$ la TM che decide $L$ in al più $O(f(n))$ passi, essendo che, ad ogni passo di computazione può riempire al più una cella del nastro di lavoro, necessariamente $M$ non riempirà più di $O(f(n))$ celle distinte, quindi $L$ è anche contenuto in $ SPACE(f(n))$.
\hfill$\blacksquare$\acc   
Il  teorema ha un significato molto chiaro:
\sapbox{lo spazio limita il tempo}
Vediamo ora in che modo il tempo può limitare lo spazio. \acc
...
$$ a_1 \ a_2 \ a_3 \ \mathbf{q_i} \ a_4 \ a_5 \ a_6 \ ;\ k$$
dove \begin{itemize}
    \item $a_j$ sono caratteri sul nastro di lavoro 
    \item $\mathbf{q_i}$ è lo stato attuale, e la sua posizione indica la posizione della testina sul nastro di lavoro 
...
Essendo $M$ deterministica, ed essendo un decisore, nessuna configurazione può ripetersi due volte durante una computazione. L'implicazione è chiara :\begin{quote}
    Il numero \textg{massimo } di passi di computazione che può fare $M$ su un qualsiasi input, è limitato dal numero totale di configurazioni, in quanto al più, una computazione può includere nella sequenza ogni singola configurazione, ma queste non possono ripetersi
...
ma $P\ne EXP$ quindi una delle inclusioni è stretta, in particolare, almeno una delle due seguenti affermazioni è vera\begin{itemize}
    \item $P\ne PSPACE$, se fosse vero che $P=PSPACE$, allora sarebbe che $P=NP$
    \item $PSPACE\ne EXP$
...
Si definisce la seguente procedura \texttt{FIND\_PATH(x,y,k)} definita come segue 
\begin{lstlisting}
FIND_PATH(x,y,k){
    if(k==0){
...
\end{lstlisting}
Al termine, si avrà che la procedura accetterà solo se esiste un cammino da $x$ ad $y$ con al più $2^k$ nodi. Sarà necessario far si che la TM $M$ esegua la procedura \begin{quote}
...
$$ C=afjej\ \mathbf{q_i} \ sfj \ ;\ r$$
dove $\mathbf{q_i}$ è lo stato, che indica anche la posizione della testina sul nastro di lavoro, ed $r$ è la posizione della testina sul nastro di input. Esistono 
$2^{O(\log(n))}=O(n)$ possibili configurazioni, si associa, per ogni coppia $N,x$ ($x$ input di $N$) un grafo $G_{N,x}$, i cui nodi sono le possibili configurazioni $C$, ed esiste un arco $(C,C')$ se $C'$ è una configurazione che può seguire da $C$ in accordo con le regole stabilite dalla funzione di transizione $\delta$. Inoltre esisterà una configurazione iniziale $C_{start}$ (naturalmente definita). 
\begin{center}
...
\end{center}
Naturalmente, $N(x)$ accetta se e solo se esiste un cammino da $C_{start}$ a $C_{acc}$ nel grafo $G_{N,x}$. Pertanto, è possibile eseguire una DFS sul grafo che ha $2^{O(\log(n))}=O(n)$ nodi, e ciò può essere fatto in tempo polinomiale, quindi $A\in NLOG \implies A\in P$, dato che esiste una $TM$ $M$ (deterministica) che decide in tempo polinomiale se $x\in A$ tramite la seguente procedura\begin{itemize}
...
Non è però possibile in questo caso scrivere il grafo $G_{N,x}$ nel nastro di lavoro in quanto violerebbe lo spazio logaritmico, ma ciò non è necessario in quanto si può valutare di volta in volta se esiste un arco $(C_i,C_j)$ valutando l'input $x$.\hfill$\blacksquare$\acc  
...
\dimo{} Sappiamo già che $PATH$ è un linguaggio in $NLOG$, bisogna dimostrare che tutti i linguaggi in tale insieme si riducono a $PATH$. Sia $A$ un qualsiasi linguaggio in $NLOG$, e sia $N$ la NTM (di complessità logaritmica) tale che $L(A)=N$\begin{itemize} \item si definisce una TM $R$ che, dato un input $x$ per $N$, costruisce il grafo $G_{N,x}$ delle configurazioni visto nella dimostrazione del teorema di Savitch. 
    \item La TM $R$ non può scrivere sul nastro l'intero grafo, ma può considerare una configurazione (nodo del grafo) alla volta 
    \item essendo che il numero  delle configurazioni è polinomiale in $n$, una configurazione può essere rappresentata utilizzando $O(\log(n))$ spazio
    \item Se $C_i$ è la configurazione corrente, è possibile verificare se esiste un arco $(C_i,C_j)$ in spazio logaritmico 
\end{itemize} 
$R(x)$ accetta se e solo se esiste il cammino da $C_{start}$ a $C_{accept}$, quindi $x\in A\iff (G_{N,x}, C_{start}, C_{acc})\in PATH$.
\hfill$\blacksquare$\acc 
...
\sapbox{\textbf{Problema aperto} : La seguente proposizione non è stata dimostrata, e tutt'ora non si sa se sia vera o falsa $$C\text{ è }P\text{-completo e } C\in LOG\implies P\subseteq LOG$$ 
...
\dimo{} Rappresentiamo una TQBF come segue 
$$ (Q_1x_1,Q_2x_2\dots , Q_nx_n,\phi(x))$$
i termini $Q_ix_i$ indicano che la variabile $x_i$ è determinata dal quantificatore $Q_i$, la formula $\phi$ invece rappresenta la formula booleana vera e propria che coinvolge le variabili quantificate.
Si considera il seguente algoritmo ricorsivo \begin{quote}
...
$$\phi_k(C_0,C_1) $$ è definita come segue : $$\exists C_{mid}\forall D\forall D'
 \begin{pmatrix}
    (D,D')=(C_0,C_{mid})\\ \lor \\ 
    (D,D')=(C_{mid},C_1)
...
Basta mostrare che $PATH\in coNLOG$, definendo un verificatore $V$ che tramite un certificato, verifica che non esiste un cammino fra due nodi in un grafo usando spazio logaritmico. Il certificato che denotiamo $y$ avrà dimensioni non logaritmiche, questo può essere quindi letto una sola volta, da sinistra verso destra.\acc 
L'input del verificatore è $<<G,s,t>,y>$, deve verificare che in $G$ non ci sia il cammino da $s$ a $t$.\acc
\textbf{Notazione} : Denotiamo $R_l$ l'insieme dei nodi raggiungibili da $s$ in al più $l$ passi, e denotiamo la sua cardinalità $r_l=|R_l|$. Chiaramente $R_n$ contiene tutti i nodi raggiungibili da $s$, inoltre $R_0=\{s\}$ quindi $r_0=1$.\acc 
Il certificato $y$ è composto da diversi sotto-certificati 
$$ y=cert(r_0) \ cert(r_1) \ cert(r_2) \dots  \ cert(r_n)$$
Ogni sotto-certificato $cert(r_l)$ sarà a sua volta composto da $n$ sotto certificati (uno per ogni nodo), in cui certificherà che un nodo è (oppure non è) contenuto in $R_l$, ad esempio 
$$ cert(r_l)=\begin{matrix}
    \text{certificato per }v_1\in R_l \\  
    \text{certificato per }v_2\notin R_l \\ \vdots \\  
    \text{certificato per }v_n\in R_l 
\end{matrix}$$
Quindi $cert(r_l)$ verifica per ogni nodo, se è o non è raggiungibile da $s$ in al più $l$ passi. Ma come può certificare ciò per ogni nodo?\begin{itemize}
    \item supponiamo che $v_i\in R_l$, allora in $cert(r_l)$ c'è il certificato 
    $$\text{certificato per }v_i\in R_l$$
    e questo non è altro che la successione di archi che compongono il cammino da $s$ a $v_i$. 
     $$\text{certificato per }v_i\in R_l=(s,v_k)(v_k,v_j)(v_j,v_p)\dots (v_b,v_i)$$
    $V$ può quindi verificare che il certificato dica la verità controllando gli archi in $G$ ed utilizzando un contatore (che necessita $\log(n)$ spazio) per verificare che questo cammino non sia più lungo di $l$.
    \item supponiamo che $v_i\notin R_l$, allora, il sotto certificato  
     $$\text{certificato per }v_i\notin R_l$$ 
     sarà composto dall'insieme dei cammini da $s$ ai nodi presenti in $R_{l-1}$
$$\text{certificato per }v_i\notin R_l=(s\rightarrow v_k)\dots , (s\rightarrow v_j)$$
chiaramente $V$ può verificare che $v_i$ non sia in $R_l$ controllando i cammini del certificato, e verificando che \begin{itemize}
    \item $v_i$ non è in $R_{l-1}$
    \item per ogni nodo  $v_j\in R_{l-1}$ non esiste l'arco $(v_j,v_i)$
\end{itemize} 
\end{itemize}
se \textbf{si assume che $cert(r_{l-1})$ sia valido}, allora $V$ può verificare in spazio logaritmico che un nodo $v_i$ non sia in $R_l$.\acc 
Quindi il verificatore partendo dall'inizio può verificare ad ogni passo $cert(r_l)$ considerando vero $cert(r_{l-1})$, in questo modo, può verificare ogni passo fino ad aver verificato $cert(r_{n-1})$, una volta arrivato a tal punto, dovrà 
verificare che $cert(r_n)$ sia valido, e che in $R_n$ non sia presente $t$.
\acc 
Un'ultima accortezza : $V$ deve verificare che i nodi coinvolti nei certificati non si ripetano, ad esempio, non deve succedere che 
$$\text{certificato per }t\notin R_n=(s\rightarrow \color{blue}v_k\color{black})\dots ,(s\rightarrow \color{blue}v_k\color{black}), (s\rightarrow v_j)$$
Quindi, si assume che i nodi nel certificato siano ordinati (ogni nodo è identificato da un numero intero), e $V$ deve assicurarsi che l'ordine sia sempre rispettato.
\hfill$\blacksquare$
\flowerLine
\section{Teoremi di Gerarchia}
I risultati presentati in questa sezione permettono di separare le classi di complessità utilizzando la tecnica della \textit{diagonalizzazione}.\acc \teo{(Idea)}
...
Lo scopo di questa dimostrazione è dimostrare la separazione fra le classi temporali\begin{quote}
 \textg{vogliamo mostrare che il linguaggio di $D$ non è in $DTIME(t_1(n))$}.\end{quote}
\textbf{Non esiste} una TM $Q$ che decide $L(D)$ in $DTIME(t_1(n))$, supponiamo per assurdo che $Q(x)$ decida $L(D)$ in tale tempo, allora, sia $Q$ che $D$ sono decisori per $L(D)$, vuol dire che per ogni $x$
$$ Q(x)=D(x)$$
Applichiamo ora la \textg{diagonalizzazione}, diamo come input a $D$ la stringa $x_Q$ tale che $[x_Q]_{TM}=Q$\begin{itemize}
    \item $D$ interpreta $x_Q$ come $Q$
    \item Esegue $Q(x_Q)$
    \item $Q(x_Q)$ termina in meno di $t_{1.5}(n)$ passi (dato che per ipotesi $Q$ ha complessità $t_1(n)$) quindi per costruzione di $D$, questa restituisce il valore opposto a $Q$
\end{itemize}
Ma allora $D(x_Q)\ne Q(x_Q)$, ciò contraddice l'assunzione, allora è \textbf{impossibile} che $Q$ decida $L(D)$ in un tempo $t_1(n)$, quindi $\exists L\in DTIME(t_2(n))$ tale che $L\notin DTIME(t_1(n))$.
\hfill$\blacksquare$\acc 
Bisogna formalizzare la relazione fra $t_1$ e $t_{1.5}$, fra queste, si ha che 
$$ t_1(n)=o(t_{1.5}(n)) \ \ \text{ ossia } \ \  \lim_{n\rightarrow \infty}\frac{t_1(n)}{t_{1.5}(n)}=0$$
C'è un problema nella dimostrazione, se la stringa $x$ presa in input da $D$ è troppo corta, la simulazione potrebbe terminare in meno di $t_{1.5}(n)$ passi a prescindere da quale sia la TM codificata, bisogna quindi adoperare una codifica  $[.]_{TM}$ che faccia del padding su ogni stringa $x$ input di $D$ rendendola sufficientemente lunga aggiungendo una serie di zeri.\acc 
...
\acc 
\defi{} Una funzione $t:\N\rightarrow \N$ è detta \textbf{tempo costruttibile} se, la funzione che associa la stringa $1^n$ (una sequenza di $n$ volte 1) alla rappresentazione binaria di $t(n)$, è computabile in tempo $O(t(n))$.\acc
\teo{di Gerarchia di Tempo} Siano\begin{itemize} \item $t_1(n)\ge n$ \item $t_2(n)= \Omega(t_1(n)\log(t_1(n)))$\end{itemize} se $t_1$ è \textit{tempo costruttibile}, esiste un linguaggio $L$ tale che $L\in DTIME(t_2(n))$ e $L\notin DTIME(t_1(n))$.\acc 
...
\teo{di Gerarchia di Spazio} Siano \begin{itemize}
    \item $s_1(n)\ge \log(n)$
    \item $s_2(n)=\Omega(s_1(n))$
\end{itemize}
esiste un linguaggio $L$ tale che $L\in SPACE(s_2(n))$ e $L\notin SPACE(s_1(n))$.\acc 
Il processo dimostrativo è identico a quello utilizzato per il teorema di gerarchia di tempo.
\acc 
I due teoremi di gerarchia si riferiscono alle classi di tempo delle macchine di Turing deterministiche. Cosa si può dire delle macchine non deterministiche? \acc 
\teo{} Siano\begin{itemize}
    \item 
 $t(n)\ge n$\item $T(n)$ una funzione tale che $t(n+1)=o(T(n))$\end{itemize}
allora $\exists L \in NTIME(T(n))$ tale che $L \notin NTIME(t(n))$\acc
\dimo{}
Si consideri un lingaggio $L$ che è sottoinsieme di $\{1\}^*$, ossia contiene solo stringhe composte da 1 (ed eventualmente la stringa vuota). Data una stringa $1^n\in L$, denotiamo $N_n$ la NTM che corrisponde a tale stringa secondo un'arbitraria codifica 
$$ [1^n]_{TM}=N_n$$
Sia $D$ la TM che deve decidere $L\subseteq \{1\}^*$, accetta quindi le stringhe del tipo $1^n$, divide l'input come segue 
$$ 1^n=[l_1,v_1][l_2,v_2]\dots [l_k,v_k]$$
dove $[l_i,v_i]$ è un intervallo della stringa, con $l_i$ e $v_i$ si indicano gli indici della stringa. Chiaramente $v_i+1=l_{i+1}$.
\begin{center}
    \includegraphics[width=0.5\textwidth ]{images/teoGerarchiaNTM.eps}
\end{center}
\redText{finire la dimostrazione}
...
Essendo che $PATH$ è $NLOG$ completo, è necessario dimostrare che $PATH$ è in $coNLOG$, ossia che $\overline{PATH}$ è in $NLOG$.
Si definisce una TM non determinista che decide $\overline{PATH}$ in spazio logaritmico. La TM è descritta dal seguente algoritmo, che ha come input il grafo $G$ ed i due nodi $s$ e $t$:\greybox{
$M(G,s,t)\{$\\ 
\hphantom{ident}$c_0=1$\\ 
\hphantom{ident}for($i=0; \ i<m; \ i$++)\{\\
\hphantom{identident}$c_{i+1}=1$\\
\hphantom{identident}per ogni nodo $v\ne s\in G$ esegui\{\hphantom{aaaaaa}\color{darkgreen}//ciclo annotato\color{black}\\
\hphantom{identidentident}$d=0$\\
\hphantom{identidentident}\color{blue}non deterministicamente esegui (e non) la procedura seguente\{\color{black}\\
\hphantom{identidentidentident}per ogni nodo $u\in G$ esegui\{\\
\hphantom{identidentidentidentident}esegui \color{blue} non deterministicamente \color{black} ogni cammino da $s$ di lunghezza al più $i$\\
\hphantom{identidentidentidentident} se non termina su $u$, rifiuta \\
\hphantom{identidentidentidentident} se  termina su $u$, incrementa $d$ \\
\hphantom{identidentidentidentident} se  $(u,v)\in G$, incrementa $c_{i+1}$ \\
\hphantom{identidentidentidentident}ricomincia il ciclo esterno (\color{darkgreen}//ciclo annotato\color{black})considerando il prossimo $v$\\
\hphantom{identidentidentident}\}\\
\hphantom{identidentidentident}se $d\ne c_i$ rifiuta\\
\hphantom{identidentident}\color{blue}\}\color{black}\\
\hphantom{identident}\}\\
\hphantom{ident}\}\\
$\}$
}
