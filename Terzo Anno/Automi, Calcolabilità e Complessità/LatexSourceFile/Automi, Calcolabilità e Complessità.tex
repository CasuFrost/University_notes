...
Ogni sotto-certificato $cert(r_l)$ sarà a sua volta composto in $n$ sotto certificati (uno per ogni nodo), in cui certificherà che un nodo è (oppure non è) contenuto in $R_l$, ad esempio 
...
Quindi $cert(r_l)$ verifica per ogni nodo, se è o non è raggiungibile da $s$ in al più $l$ passi. Ma come può certificare ciò per ogni nodo?\begin{itemize}
    ...
    e questo non è altro che la successione di archi che compone il cammino da $s$ a $v_i$. 
...
chiaramente $V$ può verificare che $v_i$ non sia in $R_l$ controllando i cammini del certificando, e verificando che \begin{itemize}
