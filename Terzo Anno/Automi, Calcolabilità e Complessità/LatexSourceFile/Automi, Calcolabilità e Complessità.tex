...
\dimo{} Sia $L\in DTIME(f(n))$, e sia $M$ la TM che decide $L$ in al più $O(f(n))$ passi, essendo che, ad ogni passo di computazione può riempire al più una cella del nastro di lavoro, necessariamente $M$ non riempirà più di $O(f(n))$ celle distinte, quindi $L$ è anche contenuti in $ SPACE(f(n))$.
\hfill$\blacksquare$\acc   
...
Quindi $cert(r_l)$ verifica per ogni nodo, se è o non è raggiungibile da $s$ in al più $l$ passi. Ma come può certificare ciò per ogni nodo?\begin{itemize}
    ...
    e questo non è altro che la successione di archi che compongono il cammino da $s$ a $v_i$. 
     $$\text{certificato per }v_i\in R_l=(s,v_k)(v_k,v_j)(v_j,v_p)\dots (v_t,v_i)$$
...
$$\text{certificato per }t\notin R_n=(s\rightarrow v_k)\dots ,(s\rightarrow v_k), (s\rightarrow v_j)$$
