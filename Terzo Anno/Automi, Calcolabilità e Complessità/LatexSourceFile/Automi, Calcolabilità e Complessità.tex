...
Si definisce poi una procedura $\text{\code{Convert}} : GNFA \rightarrow GNFA$ modifica un GNFA restituendone uno equivalente, ma con uno stato in meno. 
...
\end{cases}
$$
\item passo 4 : \text{\code{Convert}}ire le ultime regole nella forma corretta $$
\begin{cases}
    S_0 \longrightarrow ASA |  aB | SA |AS \\
    S\longrightarrow ASA |  aB | SA |AS   \\ 
    A \longrightarrow b \\
...
\teo{(di Ladner)} Se $P\ne NP$, allora esiste un linguaggio $L\in NP$ tale che $N$ non è $NP$ completo.
\flowerLine
