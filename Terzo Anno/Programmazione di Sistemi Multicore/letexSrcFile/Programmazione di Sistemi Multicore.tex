...
L'invio dei messaggi avviene tramite \code{int MPI\_Send}, i cui parametri sono\begin{itemize}
...
\subsubsection{Esempi di GSLP}\begin{itemize}
    ...
    esecuzione di un ciclo \code{for}, è necessario che le iterazioni però siano indipendenti fra loro.
\end{itemize}
\begin{lstlisting}[style=CStyle]
...
\end{center}
L'ordine di concatenazione è sempre dato dal rank. I parametri sono pressoché identici alla \code{scatter}\begin{itemize}
    ...
    \item  \code{MPI\_Datatype send\_type} il tipo degli elementi del vettore 
    \item  \code{void *recv\_buf\_p} il buffer che conterrà il vettore finale composto dalla concatenazione dei 
    vettori parziali 
    \item  \code{int recv\_count} analogo a \codee{send\_count}
    \item  \code{MPI\_Datatype recv\_type} analogo a \codee{send\_type}
...
\end{lstlisting}
Cosa va definito nel campo \code{data\_type} quando si 
...
una sequenza di istruzioni assembly che possono essere interfogliate dai due thread.\begin{center}
    \begin{tabular}{ccc}
        ...
        \multicolumn{1}{|c|}{1}                                        & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}inizializzato dal \\ main thread\end{tabular}}                                                                      & \multicolumn{1}{c|}{}                                                                                                                                             \\ \hline
        \multicolumn{1}{|c|}{2}                                        & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}legge la variabile \texttt{x=0}\\ e la carica nel registro r1\end{tabular}}                       & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}inizializzato dal \\ main thread\end{tabular}}                                                                      \\ \hline
...
Se due o più thread eseguono un operazione di \code{member} contemporaneamente, il programma funziona senza problemi, ma cosa succederebbe se contemporaneamente, un thread eseguisse un operazione di \code{member}  ed un altro un operazione di modifica? L'accesso alla lista potrebbe non essere consistente.
\begin{center}
\begin{tabular}{c|c|c|}
    \cline{2-3}
...
\end{lstlisting}
Questo approccio è più complesso ed appesantisce notevolmente la lista, in quanto ogni nodo occuperà più byte in memoria, inoltre, ad ogni singolo accesso ci sarà un \code{lock} seguito da un \code{unlock}, rallentando significativamente le operazioni, gestire tali sequenze di accessi non è semplice e si può facilmente incombere in deadlock.\acc 
...
\end{center}
\begin{itemize}
    \item Prima chiamata \code{strtok(string, " ")} ritorna "\texttt{live}".
    \item Seconda chiamata \code{strtok(string, NULL)} ritorna "\texttt{and}".
    \item Terza chiamata \code{strtok(string, NULL)} ritorna "\texttt{learn}".
\end{itemize}
...
$$F_n=\frac{\varphi^n-(1-\varphi)^n}{\sqrt{5}} \ \ \ \  \text{ dove } \ \ \ \ \frac{1}{\varphi}\simeq 0.618$$ 
\begin{lstlisting}[style=CStyle]
fibo[0]=fibo[1]=1;
#pragma omp parallel for 
for(int i=2;i < N;i++){
...
\shell{export OMP\_SCHEDULE = "static, 1"}
...
Per le sezioni critiche è meglio utilizzare \texttt{critical}, \texttt{atomic} oppure le lock?\begin{enumerate}
    ...
    \item  L'uso dei lock dovrebbe probabilmente essere riservato a situazioni in cui la mutua
    esclusione è necessaria per una struttura di dati piuttosto che per un blocco di codice 
\end{enumerate}
\subsection{Sezioni Parallele}
Le direttive \texttt{master} e \texttt{single} in OpenMP sono strumenti utili per controllare l'esecuzione di specifiche sezioni di codice all'interno di un costrutto parallelo.  Entrambe limitano l'esecuzione di un blocco di codice a un singolo thread, ma lo fanno in modi leggermente diversi.\begin{itemize}
...
Le direttive \texttt{master} e \texttt{single} precludono ad alcuni thread l'esecuzione di una sezione, esiste anche la direttiva \texttt{\#pragma omp parallel section} che equivale ad uno \texttt{switch} parallelo, ossia permette di specificare ai thread un comporamento diverso da eseguiro in parallelo agli altri.
\begin{lstlisting}[style=CStyle]
...
Abbiamo visto come \begin{itemize}
   \item Una GPU è composta da più core disposti su un SM, e più SM formano un blocco condividendo la cache
   \item I thread di CUDA sono organizzati logicamente in una struttura 6-dimensionale composta da blocchi su una griglia
\end{itemize}
\sapbox{\textbf{Attenzione} : I blocchi fisici della GPU (2 o più SM) ed i blocchi logici di CUDA (elementi della griglia) \textit{non sono la stessa cosa}, seppur condividano il nome "blocco".}
...
Ci sono differenti tipi di memoria in una GPU\begin{center}
    \includegraphics[width=\textwidth ]{images/memoryTypesAI.png}
\end{center}
\begin{itemize}
...
\subsection{Memoria Globale}
La memoria globale è divisa in banchi con gli indirizzi assegnati ai banchi in modo ciclico, ad
esempio con 16 banchi:\begin{center}
    \includegraphics[width=1\textwidth ]{images/banco.png}\end{center}
    Se due thread accedono a due banchi di memoria diversi l'accesso è istantaneo, se invece accedono allo stesso banco gli accessi saranno serializzati, obbligando uno dei due thread ad aspettare
...
Quando viene caricato un valore dalla memoria globale, viene caricato quello che viene chiamato un \text{burst}, cioè un insieme di indirizzi consecutivi a quello caricato e se tutti thread in
un warp accedono a locazioni di memoria consecutive l'hardware \textbf{”coalesces”} gli accessi, cioè
...
accessi in locazioni lontane tra loro per evitare di caricare molti valori inutili.\flowerLine
